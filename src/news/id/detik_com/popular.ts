...
const listPageItemsSelector =
  '//div[contains(@class, "grid-row") and contains(@class, "list-content")]//article';
      ...
      await Promise.allSettled(
        Array.from({ length: countListPageItems }, (_, i) => i).map(
          async (index) => {
            ...
            if (detailLocalCategory === null) {
              if (listLink?.includes("wolipop.detik.com/")) {
                detailLocalCategory = "Wolipop";
              }
            }

            if (detailLocalCategory === null) {
              if (listLink?.includes("travel.detik.com/")) {

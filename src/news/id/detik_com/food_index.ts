...
const baseURLPath = "https://food.detik.com/indeks/";
        ...
        Array.from({ length: countListPageItems }, (_, i) => i).map(
          async (index) => {
            ...
            if (detailLocalCategory === null) {
              detailLocalCategory = getAttributeFromElement(
                detailPage,
                'meta[name="namaprogram"]',
                "content",
              );
            }
            ...
            if (detailPublishedDateTime === null) {
              detailPublishedDateTime = getAttributeFromElement(
                detailPage,
